...
$dnsHostnames = '@', 'DomainDnsZones', 'ForestDnsZones'
foreach ($dnsHostname in $dnsHostnames) {
    $removeDnsServerResourceRecordSplat = @{
        Name        = $dnsHostname
        RRType      = 'A'
        ZoneName    = $DomainName
        RecordData  = $ipAddressToRemove
        Force       = $true
        ErrorAction = 'SilentlyContinue'
        Verbose     = $true
    }
    Remove-DnsServerResourceRecord @removeDnsServerResourceRecordSplat
}
